...
sdk/py/
├── m8tes/
│   ├── client.py         # Core HTTP client + auth middleware
│   ├── agent.py          # Mate models + execution helpers
│   ├── cli/
│   │   ├── main.py       # Entry point (`m8tes`)
│   │   └── commands/     # auth, mate task/chat, scheduling commands
│   ├── auth/             # Credential store + login flows
│   ├── streaming.py      # SSE parsing utilities
...
client = M8tes(api_url="http://127.0.0.1:8000")
