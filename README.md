...
Today that's the Claude models `sonnet`, `opus`, and `fable` (most capable, ~2x cost); OpenAI `gpt-5.5`, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`; Google `gemini-3.7-flash` / `gemini-3.6-flash`; xAI `grok-4.6` (**platform default**) / `grok-4.5`; and open-source `glm-5.2` / `minimax-m3` / `deepseek-v4-pro-0813` / `deepseek-v4-pro` / `deepseek-v4-flash-0731` (lowest cost) / `deepseek-v3-2` / `kimi-k2-7-code` / `kimi-k3` / `qwen3.8-max`. Zero-data-retention support is **per model and changes over time**, so never assume it from the model name: read `zdr_supported` on `GET /api/v2/models`, or filter with `?zdr=true`, before sending customer data — `claude-fable-5` in the curated picker is the documented exception with no ZDR host. `models.list()` is the live source of truth; omit `model` to use the `default`.
